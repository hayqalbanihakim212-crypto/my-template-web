...
app.post('/api/admin/data', (req, res) => {
    try {
        if (req.body.password !== 'admin123') return res.status(401).json({ success: false, message: 'Password salah!' });
        const users = readJSON(DB_FILE);
...
app.post('/api/admin/upload', upload.single('file'), (req, res) => {
    try {
        ...
        if (password !== 'admin123') return res.status(401).json({ success: false, message: 'Password salah!' });
